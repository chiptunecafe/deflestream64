...
.macro MAYBEGETBLOCK
	.if BYTESTREAM
		...
		lda decompgetbyte + $01; no block polling when loading
		eor #.lobyte(getcload) ; via KERNAL fallback
		bne :+
		.endif
...
.if KERNOPT
         jsr pollblock
    .if (LOAD_UNDER_D000_DFFF) & (PLATFORM <> diskio::platform::COMMODORE_16)
         ENABLE_ALL_RAM; .lz_refill_bits loads bytes ahead when going via .lz_next_page, but may also go beyond EOF, which
                       ; yields an error and restores the previos memory configuration: ignore the error and set all-RAM
                       ; memory configuration, so that the rest of the file can be depacked correctly to RAM under IO
    .endif
         tya
         eor #$ff
         tax
